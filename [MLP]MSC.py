...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        for i, (mnist_data, svhn_data, cifar_data) in enumerate(train_loader_zip):
        ...
        print(f'  [Acc]    MNIST: {mnist_acc_epoch:<6.2f}% | SVHN: {svhn_acc_epoch:<6.2f}% | CIFAR: {cifar_acc_epoch:<6.2f}%')
        ...
        with ((torch.no_grad())):
            ...
            test_label_avg_loss = (test_mnist_avg_loss + test_svhn_avg_loss + test_cifar_avg_loss) / 3.0
            test_total_avg_loss = test_label_avg_loss
            current_avg_acc = (test_mnist_acc_epoch + test_svhn_acc_epoch + test_cifar_acc_epoch) / 3.0

            print(f'Epoch [{epoch + 1}/{num_epochs}] (Test)')
            print(f'  [Acc]    MNIST: {test_mnist_acc_epoch:<6.2f}% | SVHN: {test_svhn_acc_epoch:<6.2f}% | CIFAR: {test_cifar_acc_epoch:<6.2f}%')
            print(f'  [Loss]   Label: {test_label_avg_loss:<8.4f} | Total: {test_total_avg_loss:<8.4f}')
            print(f'  [Label]  MNIST: {test_mnist_avg_loss:<6.4f} | SVHN: {test_svhn_avg_loss:<6.4f} | CIFAR: {test_cifar_avg_loss:<6.4f}')
